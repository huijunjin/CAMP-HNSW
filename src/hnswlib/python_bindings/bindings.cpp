...
PYBIND11_PLUGIN(hnswlib) {
        ...
        // ---- CAMP-HNSW extensions ----

        .def("inject_shortcuts_binary", [](Index<float> &index, const std::string &location) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            index.appr_alg->injectShortcutsBinary(location);
        }, py::arg("location"), "Inject OOD shortcuts directly into the HNSW graph (Level 0) from a binary file")

        .def("reset_performance_metrics", [](Index<float> &index) {
            if (index.appr_alg) {
                index.appr_alg->metric_distance_computations = 0;
                index.appr_alg->metric_hops = 0;
            }
        })

        .def("get_performance_metrics", [](Index<float> &index) {
            if (index.appr_alg) {
                return py::dict(
                    "visited"_a = (long)index.appr_alg->metric_distance_computations,
                    "hops"_a = (long)index.appr_alg->metric_hops
                );
            }
            return py::dict("visited"_a = 0, "hops"_a = 0);
        })

        .def("get_orphan_count", [](Index<float> &index) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            auto alg = static_cast<hnswlib::HierarchicalNSW<float>*>(index.appr_alg);
            return alg->getOrphanNodeCount();
        }, "Get the number of orphan nodes (in-degree == 0)")

        .def("get_orphan_nodes", [](Index<float> &index) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            auto alg = static_cast<hnswlib::HierarchicalNSW<float>*>(index.appr_alg);
            return alg->getOrphanNodes();
        }, "Get the list of orphan node IDs")

        .def("enable_profiling", [](Index<float> &index) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            auto alg = static_cast<hnswlib::HierarchicalNSW<float>*>(index.appr_alg);
            alg->enable_profiling();
        }, "Enable edge activity profiling before test queries")

        .def("get_profiling_stats", [](Index<float> &index) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            auto alg = static_cast<hnswlib::HierarchicalNSW<float>*>(index.appr_alg);
            return py::dict(
                "eval_counts"_a = py::cast(alg->node_eval_counts),
                "hop_counts"_a = py::cast(alg->node_hop_counts),
                "eval_base"_a = (long long)alg->metric_eval_base_edges,
                "eval_shortcut"_a = (long long)alg->metric_eval_shortcut_edges,
                "hop_base"_a = (long long)alg->metric_hop_base_edges,
                "hop_shortcut"_a = (long long)alg->metric_hop_shortcut_edges
            );
        }, "Get profiling stats as a python dictionary")

        .def("get_injection_stats", [](Index<float> &index) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            auto alg = static_cast<hnswlib::HierarchicalNSW<float>*>(index.appr_alg);
            return py::dict(
                "original_edges"_a = alg->stats_original_hnsw_edges,
                "shortcuts_tried"_a = alg->stats_shortcuts_tried,
                "shortcuts_survived"_a = alg->stats_shortcuts_survived,
                "hnsw_edges_deleted"_a = alg->stats_hnsw_edges_deleted,
                "final_edges"_a = alg->stats_final_edges,
                "pure_inject_time"_a = alg->stats_pure_inject_time
            );
        }, "Get statistics of shortcut injection")

        .def("import_ngfix_edges", [](Index<float> &index, const std::string& filepath, int ep, py::object old_to_new_obj, py::object new_to_old_obj) {
            if (!index.appr_alg) throw std::runtime_error("Index is not initialized");
            auto alg = static_cast<hnswlib::HierarchicalNSW<float>*>(index.appr_alg);

            py::array_t<int, py::array::c_style | py::array::forcecast> old_to_new(old_to_new_obj);
            py::array_t<int, py::array::c_style | py::array::forcecast> new_to_old(new_to_old_obj);

            std::vector<int> vec_old2new((int*)old_to_new.data(), (int*)old_to_new.data() + old_to_new.size());
            std::vector<int> vec_new2old((int*)new_to_old.data(), (int*)new_to_old.data() + new_to_old.size());

            alg->importEdgesFromNGFix(filepath, ep, vec_old2new, vec_new2old);
        }, "Replace internal graph with NGFix edges and apply ID mapping")
        //end
