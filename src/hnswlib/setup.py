...
class BuildExt(build_ext):
    ...
    # '-march=native' alone already enables the best instruction set the
    # *build machine* actually supports (AVX-512 included, on hardware like
    # the paper's Xeon w5-2465X). We deliberately do NOT also pass explicit
    # '-mavx512f/-cd/-bw/-dq' flags: those would let the compiler emit
    # AVX-512 instructions unconditionally, which compiles fine but crashes
    # with SIGILL at runtime on any reviewer machine without AVX-512.
    c_opts = {
        'msvc': ['/EHsc', '/openmp', '/O2'],
        'unix': [
            '-Ofast',
